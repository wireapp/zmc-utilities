...
class ZMEmailAddressValidatorTests: ZMTBaseTest {

    let usernameValidCharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ!#$%&'*+-/=?^_`{|}~abcdefghijklmnopqrstuvwxyz0123456789"
    let usernameValidCharactersLowercased = "abcdefghijklmnopqrstuvwxyz!#$%&'*+-/=?^_`{|}~abcdefghijklmnopqrstuvwxyz0123456789"
    let domainValidCharacters = "abcdefghijklmnopqrstuvwxyz-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    let domainValidCharactersLowercased = "abcdefghijklmnopqrstuvwxyz-0123456789abcdefghijklmnopqrstuvwxyz"
    ...
    func testThatValidEmailsPassValidation() {
        ...
        for email in validEmailAddresses {
            ...
            isValidWithNonMutableCheck = ZMEmailAddressValidator.isValidEmailAddress(validatedEmail as! String)
            validatedEmails.append(validatedEmail as! String)
            XCTAssertTrue(isValid)
            XCTAssertTrue(isValidWithNonMutableCheck)
        }
        
        validatedEmails.forEach { XCTAssertTrue(validEmailAddresses.values.contains($0))
        }
